...
#adds all the letters from ascii_lowercase to the letter_list
for letter in ascii_lowercase:
    letter_list += letter

#same as above but with the digits
...
for i in range(37):
    letter_to_morse.update({f"{letter_list[i]}":f"{morse_code[i]}"})
    morse_to_letter.update({f"{morse_code[i]}":f"{letter_list[i]}"})


while True:
    #Asks if you want to convert text to morse code or morse code to text
    text_to_morse = input("Text to Morse Code (true or false):")
    
    #acts accordingly to the response of text_to_morse and asks for what you want to translate
